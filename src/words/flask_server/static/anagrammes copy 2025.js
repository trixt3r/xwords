...
//####################################################
//ci-dessus, problemes de 2020, résolu grace à la librairie unicharadata.js
// reste plus qu'à intégrer ça dans le reste
//********************************/
...
//list of words already chosen by user
  ...
  //NOTE à utiliser là où c'est nécessaire
  function split_api(api_string){
    var ret_array = [];
    for(i=0;i<api_string.length;i++){
      categ = unicharadata.category(api_string.charAt(i));
      if(categ=="Ll" && i<api_string.length-1 && unicharadata.category(api_string[i+1])=="Mn"){
        ret_array.push(api_string.substr(i,2));
        i++;
      }else{
        ret_array.push(api_string[i]);
      }
    }
    return ret_array;
  }
  ...
  function chose_word(w){
    ...
    if (window.play_mode=="A"){
      ...
      alert("sorted chosen word: " + sorted_chosen_word);
      for(i=0;i<sorted_chosen_word.length;i++){
        alert(sorted_chosen_word[i]+" "+sorted_chosen_word.charCodeAt(i));
      }
      alert(sorted_chosen_word.length);
    ...
    if(reste==false){
      ...
      rows.each(function(){
        ...
        if(window.play_mode=="A"){
          ...
          word_row = word_row.substring(1, word_row.length-1);
        }
