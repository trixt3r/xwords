...
def extract_flextable(flex_t):
    ...
    if len(rows)==2:
        ...
        if _text=="au singulier uniquement":
            ...
            api = _extract_api(anchors[1].span.text.strip())
        ...
        #MASCULIN
            ...
            m_s_api = _extract_api(anchors[-1].text)
            if len(anchors)==3:
            ...
            if len(current_row_cells) == 1:
                ...
                m_p_api = _extract_api(anchors[-1].text)
        #FEMININ
            ...
            f_s_api = _extract_api(anchors[-1].text)
            anchors = current_row_cells[1].find_all("a")
            f_p_ort = anchors[0].text
            f_p_api = _extract_api(anchors[-1].text)
            return({
                "ms":(m_s_ort,m_s_api),
                "mp":(m_p_ort,m_p_api),
                "fs":(f_s_ort,f_s_api),
        ...
        if "invariable" in rows[0].th.text.lower():
            ...
            api = _extract_api(rows[2].a.span.text)
                ...
                if nature in ["nom", "adj"]:
                    ...
                    if flextable is None:
                        ...
                        if len(emplois)==1:
                            ...
                            genre = None
                            nombre = None
                            t = " ".join([t.text for t in block.find_all("span", class_="ligne-de-forme")])
                            if "invariable" in t:
                                nombre = "I"
                            elif "singulier" in t:
                                nombre = "S"
    ...
    def extract_temps(table):
        ...
        if len(lines) == 7 or len(lines) == 4:
            for line in lines[1:]:
                ...
                api = _extract_api(tds[i2].span.text)
...
def extract_verb_info_wiki(v:str):
    ...
    def extract_conjug_form(conjug,pronominale=False):
        ...
        while not infinitif_html.name == 'tr':
            infinitif_html = infinitif_html.parent
        infinitif_html = infinitif_html.find_all("td")
        api_infinitif = _extract_api(infinitif_html[3].text.strip())
    ...
    if len(types_conjug)==2:
        if "active" in types_conjug[0][0] and "pronominale" in types_conjug[1][0]:
            ...
            assert forme1.startswith("\\") and forme1.endswith("\\") and forme2.startswith("\\") and forme2.endswith("\\"), f"j'attendais 2 prononciations différentes pour le verbe {v}"
    ...
    if len(conjug_divs)>2:
        raise Exception(f"trop de formes de conjugaison pour {v}")
    # conjug = None
