...
class Node(object):
    ...
    def get_children_keys_iter(self):
        keys = [x for x in self.children.keys()]
        keys.sort()
        for k in keys:
            yield k
        return
...
# parcours prefix. node.data is a list
def parcours_prefix_arbre_data_list(node, filter=lambda x: True):
    fifo = [node]
    while len(fifo) > 0:
        c_node = fifo.pop()
        if c_node.data is not None:
            for k in c_node.data:
                if filter(k):
                    yield k
        next = [c for c in c_node.children]
        next.sort(reverse=True)
...
# parcours prefix. node.data is a dict
def parcours_prefix_arbre_data_dict(node, filter=lambda x: True):
    fifo = [node]
    while len(fifo) > 0:
        c_node = fifo.pop()
        if c_node.data is not None:
